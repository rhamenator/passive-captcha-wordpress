 ...
 * Description: Configurable passive CAPTCHA for any WordPress form with timing, nonce, session, IP/UA checks, rate limiting, webhooks, IP lists, conditional JA3, improved IP detection, and admin log viewer. Requires manual integration.
...
function pch_log_event($message) {
    ...
    array_unshift($logs, "[$timestamp] " . $message);
    ...
    if (is_multisite()) {
        ...
        update_option($log_option_key, $logs, false); // No autoload
    }
}
...
function pch_enqueue_scripts() {
    // if (is_page() || is_singular()) { // Optionally scope script loading
        ...
        wp_enqueue_script('passive-captcha-hardened', plugin_dir_url(__FILE__) . 'js/passive-captcha.js', [], '4.3', true);
...
function pch_verify_submission() {
    ...
    if (pch_is_ip_blacklisted($ip)) {
        pch_log_event("Generic Failure: IP Blacklisted. IP: {$ip}, UA: {$ua}");
    ...
    if (!empty($ja3_fingerprint)) {
        ...
        if (strlen($ja3_fingerprint) < $min_ja3_len) {
             ...
             pch_log_event("Generic Failure: JA3 Invalid Format. IP: {$ip}, UA: {$ua}, JA3: {$ja3_fingerprint}");
    ...
    if (pch_check_rate_limit($ip)) {
        pch_log_event("Generic Failure: Rate Limit Exceeded. IP: {$ip}, UA: {$ua}");
    ...
    if (!wp_verify_nonce($submitted_nonce, 'pch_captcha_nonce')) {
        pch_register_failure($ip); pch_log_event("Generic Failure: Nonce Invalid. IP: {$ip}, UA: {$ua}");
    ...
    if (empty($submitted_session) || !get_transient($session_transient_key)) {
        pch_register_failure($ip); if ($submitted_session) delete_transient($session_transient_key);
        pch_log_event("Generic Failure: Session Invalid/Expired. IP: {$ip}, UA: {$ua}, Session: {$submitted_session}");
    ...
    if ($submitted_iphash !== $expected_iphash) {
        pch_register_failure($ip); delete_transient($session_transient_key);
        pch_log_event("Generic Failure: IP/UA Mismatch. IP: {$ip}, UA: {$ua}, Submitted: {$submitted_iphash}, Expected: {$expected_iphash}");
    ...
    if (empty($submitted_value) || $submitted_value === 'no_interaction') {
        pch_register_failure($ip); delete_transient($session_transient_key);
        pch_log_event("Generic Failure: No Interaction/JS Fail. IP: {$ip}, UA: {$ua}, Token: {$submitted_value}");
    ...
    if ($decoded === false || strpos($decoded, ':') === false) {
        pch_register_failure($ip); delete_transient($session_transient_key);
        pch_log_event("Generic Failure: Invalid Token Format. IP: {$ip}, UA: {$ua}, Token: {$submitted_value}");
    ...
    if (!is_numeric($timeSpent) || $timeSpent < $min_time || strlen($navigatorHash) < $min_hash_len) {
        pch_register_failure($ip); delete_transient($session_transient_key);
        pch_log_event("Generic Failure: Timing/Fingerprint Invalid. IP: {$ip}, UA: {$ua}, Time: {$timeSpent} (Min: {$min_time}), HashLen: " . strlen($navigatorHash) . " (Min: {$min_hash_len})");
        return new WP_Error('timing_or_fingerprint_invalid', $user_error_message);
    }

    // --- ALL CHECKS PASSED ---
    delete_transient($session_transient_key);
    // pch_log_event("Generic Success: Verification Passed. IP: {$ip}");
    return true;
}
    ...
    if (isset($_POST['pch_clear_log']) && isset($_POST['pch_clear_log_nonce']) && wp_verify_nonce($_POST['pch_clear_log_nonce'], 'pch_clear_log_action')) {
        if (current_user_can($capability)) {
            ...
            $log_cleared_message = __('Recent log entries cleared.', 'passive-captcha-hardened');
             ...
             $log_cleared_message = __('You do not have permission to clear logs.', 'passive-captcha-hardened');
        }
